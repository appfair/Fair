...
final class GitHubEndpointServiceTests: XCTestCase {
...
//        }
    ...
    static func hub(skipNoAuth: Bool = false) throws -> GitHubEndpointService {
        if skipNoAuth == true && Self.authToken == nil {
            throw XCTSkip("cannot run API tests without a token")
        }
        return try GitHubEndpointService(hostOrg: "github.com/" + appfairName, authToken: authToken, fairsealIssuer: "appfairbot", fairsealKey: nil, requestRetryCount: 5)
    }
    ...
    func request<A: APIRequest>(_ request: A) async throws -> A.Response? where A.Service == GitHubEndpointService {
    ...
    func testQueryError() async throws {
        ...
        do {
            do {
                let response = try await hub.request(GitHubEndpointService.LookupPRNumberQuery(owner: "xxx", name: "xxx", prid: -1))
        ...
        do {
            let response = try await hub.request(GitHubEndpointService.LookupPRNumberQuery(owner: "", name: "", prid: 1))
            XCTAssertNil(response.result.successValue, "request should not have succeeded")
            if response.result.failureValue?.isRateLimitError != true {
                let reason = response.result.failureValue?.firstFailureReason
    ...
    func testFetchRepositoryQuery() async throws {
        let hub = try Self.hub(skipNoAuth: true)
        let response = try await hub.request(GitHubEndpointService.RepositoryQuery(owner: appfairName, name: baseFairgroundRepoName))
        do {
    ...
    func testFindPullRequestsQuery() async throws {
        let hub = try Self.hub(skipNoAuth: true)
        do {
            let response = try await hub.request(GitHubEndpointService.FindPullRequests(owner: appfairName, name: baseFairgroundRepoName, state: .CLOSED, count: 99))
            let content = try response.get().data
    ...
    func testLookupPRNumberQuery() async throws {
        let hub = try Self.hub(skipNoAuth: true)
        let response = try await hub.request(GitHubEndpointService.LookupPRNumberQuery(owner: appfairName, name: baseFairgroundRepoName, prid: 1)).get().data
    ...
    func testFetchCommitQuery() async throws {
        let hub = try Self.hub(skipNoAuth: true)
        let response = try await hub.request(GitHubEndpointService.GetCommitQuery(owner: "appfair", name: "Fair", ref: "93d86ba5884772c8ef189bead1ca131bb11b90f2")).get().data
    ...
    func testCatalogQuery() async throws {
        ...
        var resultResults: [[GitHubEndpointService.BaseFork]] = []
        let results = hub.requestBatchedStream(GitHubEndpointService.CatalogForksQuery(owner: appfairName, name: baseFairgroundRepoName, count: Int.random(in: 8...18)))
    ...
    func testSemanticForkIndex() async throws {
        ...
        let hub = try Self.hub(skipNoAuth: true)
        for try await batch in hub.requestBatchedStream(GitHubEndpointService.SemanticForksQuery(owner: "appfair", name: "App")) {
