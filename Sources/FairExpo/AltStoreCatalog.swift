...
public struct AltCatalogNewsItem : Codable, Equatable {
...
public struct AppCatalogSource : RawCodable, Equatable {
    public typealias RawValue = Either<AltCatalog>.Or<URL>
    ...
    public init(catalog: AltCatalog) {
    ...
    public func fetchCatalog(with session: @autoclosure () -> URLSession = URLSession.shared) async throws -> AltCatalog {
        switch rawValue {
        case .a(let catalog): return catalog
        case .b(let url): return try AltCatalog.parse(jsonData: await session().fetch(request: URLRequest(url: url)).data)
        }
    }
}
